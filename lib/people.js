 ...
 * 3.1, 3.5, 4, or 5, or the Historical Touches log.
 *
 * BM26090602: DIA dossiers (and analysis.md) moved off circle's local disk
 * onto vault's encrypted store -- readDiaFile/writeDia/readAnalysisFile are
 * async, injected functions backed by store.rawRead/rawWrite (see
 * circle/src/server.js's wiring) against `circle/dia/<id>.md` /
 * `circle/analysis.md`, the same raw-blob mechanism vault already exposes
 * for other non-tabular content (course lesson markdown). This closes the
 * "dia files are local-only, not part of vault's sync" gap this file's own
 * header used to flag.
 */
 ...
 *  encrypted store (same rawRead/rawWrite pattern people.tsv already uses
 *  for tabular data) -- readDiaFile/writeDia are now async, injected
 *  functions backed by store.rawRead/rawWrite against `circle/dia/<id>.md`,
 *  not fs. */
async function writeDiaFileForPerson({ writeDia, readDiaFile, name, id, sections }) {
  let content = (await readDiaFile(id)) || SDIAIF_V21_SKELETON(name || id);
...
function createPeopleClient(opts) {
  const {
    ...
    readDiaFile = async (id) => null,              // async (id) => string|null -- injected, BM26090602: vault raw-blob backed
    writeDia = async (id, content) => { throw new Error('createPeopleClient requires writeDia to write a DIA dossier'); },
  ...
  async function listPeople() {
    ...
    return Promise.all(people.map(async p => {
      ...
      const dia = await readDiaFile(p.ID);
  ...
  async function readDia(id) {
    ...
    return { content: (await readDiaFile(id)) || '' };
  }

  async function readAnalysis() {
    return { content: (await readAnalysisFile()) || '' };
  }
  ...
  async function whoCan(q) {
    ...
    // Fetch every person's DIA content up front (async, over the wire) so
    // the scoring pass below can stay a plain synchronous .map().
    const diaById = new Map(await Promise.all(
      people.map(async p => [p.ID, ((await readDiaFile(p.ID)) || '').toLowerCase()])
    ));
  ...
  async function writeDiaFile(id, sections) {
    ...
    return writeDiaFileForPerson({ writeDia, readDiaFile, name: person && person.NAME, id, sections });
  }
  ...
  async function currentDiaSections(id) {
    return extractCurrentSections(await readDiaFile(id));
  }
