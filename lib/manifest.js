...
module.exports = {
  ...
  capabilities: [
    ...
    // BM26090602: Teams/Channels webhook bridge -- a dedicated channel per
    // external brief recipient, a scoped per-brief token embedded in that
    // week's shareable link, and an inbound endpoint the recipient's hosted
    // HTML page calls to post a response back into the same thread.
    { name: 'teams.channel.ensure', method: 'POST', path: '/teams/channel', description: 'Get-or-create the Team/Channel for one external recipient (person_id) -- idempotent, called on first send.' },
    { name: 'teams.messages.list', method: 'GET', path: '/teams/messages', description: 'A channel\'s chronological message log (?teamId=).' },
    { name: 'teams.message.post', method: 'POST', path: '/teams/message', description: 'Post one message (brief_sent/reply_received/note) into a channel.' },
    { name: 'teams.brief-token.issue', method: 'POST', path: '/teams/brief-token', description: 'Mint a scoped, single-purpose token for one brief-send, valid only against that team+brief (default 14-day expiry).' },
    { name: 'teams.send-brief', method: 'POST', path: '/teams/send-brief', description: 'Combined outbound step: ensure channel + issue token + post the outbound message, in one call.' },
    { name: 'webhook.brief-response', method: 'POST', path: '/webhook/brief-response', description: 'INBOUND, unauthenticated (the per-brief token in the body is the auth, not CIRCLE_TOKEN) -- a recipient\'s response, verified against its scoped token and appended to the matching channel thread.' },
  ],
