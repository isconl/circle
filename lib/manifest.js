...
module.exports = {
  ...
  capabilities: [
    ...
    { name: 'circle.dia.regenerate', method: 'POST', path: '/people/:id/regenerate-dia', description: 'Admin on-demand dossier regeneration for one person (FM26082801) -- same regenerateDia() spark call as the real inbox/chat-import event hooks, callable directly instead of only as their side effect.' },
    { name: 'circle.tags.list', method: 'GET', path: '/tags', description: 'BM26091204: every distinct tag in use, with how many contacts carry it.' },
    { name: 'circle.tags.add', method: 'POST', path: '/tags/add', description: 'BM26091204: add one tag to one person (personId, tag) -- a tag "exists" only by being attached to a contact.' },
    { name: 'circle.tags.remove', method: 'POST', path: '/tags/remove', description: 'BM26091204: remove one tag from one person only (personId, tag).' },
    { name: 'circle.tags.rename', method: 'POST', path: '/tags/rename', description: 'BM26091204: rename a tag across every contact that carries it (from, to).' },
    { name: 'circle.tags.merge', method: 'POST', path: '/tags/merge', description: 'BM26091204: combine one or more source tags into a single target tag across every contact (sources, into).' },
    { name: 'circle.tags.delete', method: 'POST', path: '/tags/delete', description: 'BM26091204: remove a tag from every contact that carries it (tag).' },
    ...
    { name: 'circle.chat.import.add-sender', method: 'POST', path: '/chat-import/add-sender', description: 'BM26090503: create a Circle person for an unmatched chat-import sender and settle their messages into interactions/inbox, using the still-in-memory parsed archive from that import (importId).' },
    { name: 'circle.chat.import.confirm-suggestion', method: 'POST', path: '/chat-import/confirm-suggestion', description: 'BM26091205: confirm a fuzzy match suggestion -- links an unmatched speaker\'s messages to an EXISTING contact (personId) instead of creating a new one. Suggest-and-confirm only, never automatic.' },
    { name: 'circle.chat.import.dismiss-suggestion', method: 'POST', path: '/chat-import/dismiss-suggestion', description: 'BM26091205: dismiss a fuzzy match suggestion -- remembered so the same (incoming name, contact) pair is never re-suggested.' },
