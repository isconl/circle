    ...
    for (const m of matched.slice(0, 6)) {
      ...
      if (r?.ok) {
        ...
        if (last?.date && /^\d{4}-\d{2}-\d{2}$/.test(last.date)) {
          const existing = readTSV(interactionsFile);
          const key = `chatimport:${m.person.ID}:${last.date}`;
          if (!existing.some(x => (x.SUMMARY || '').includes(key))) {
            appendTSV(interactionsFile, { ID: `X${Date.now()}${results.length}`, PERSON_ID: m.person.ID,
            ...
            rewriteTSV(peopleFile, rows => rows.map(row =>
