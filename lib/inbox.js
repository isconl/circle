'use strict';
/**
 * Inbox: real inbound messages, verbatim, with Peter's controls. Ported
...
function createInboxClient(opts) {
  ...
  async function addMessage(p) {
    ...
    // itself -- Sconl curates, never double-enters.
    if (row.SENDER && row.SENDER !== '-' && row.SENDER !== 'iSconl') {
      ...
      if (!match) {
        ...
        if (id && !people.some(pp => pp.ID === id)) {
          appendTSV(peopleFile, { ID: id, NAME: row.SENDER, CIRCLE: 'social',
      ...
      if (match) {
        ...
        appendTSV(interactionsFile, { ID: `X${Date.now()}`, PERSON_ID: match.ID, DATE: date,
        ...
        rewriteTSV(peopleFile, rws => rws.map(r => r.ID === match.ID ? { ...r, LAST_TOUCH: date } : r));
  ...
  function updateMessage(p) {
    ...
    let found = false;
    rewriteTSV(inboxFile, rows => rows.map(r => {
  ...
  function deleteMessage(id) {
    const removed = rewriteTSV(inboxFile, rows => rows.filter(r => r.ID !== id));
