...
function createJournalClient(opts) {
  ...
  async function addEntry(p) {
    ...
    const row = {
      ...
      BODY: tsvEscapeText(body).slice(0, 20000), AI_NOTE: '-', CREATED_AT: new Date().toISOString(), EDITED_AT: '-',
    };
  ...
  /** Edit an existing entry's text/mood/energy/tags, stamping EDITED_AT --
   *  never CREATED_AT or DATE, which stay the entry's original moment. */
  async function updateEntry(p) {
    const id = String(p.id || '').trim();
    if (!id) throw new Error('id required');
    const body = p.body !== undefined ? String(p.body || '').trim() : undefined;
    if (body !== undefined && !body) throw new Error('an empty entry is not an entry');
    let found = false;
    await rewriteTSV(journalFile, rows => rows.map(r => {
      if (r.ID !== id) return r;
      found = true;
      return {
        ...r,
        ...(body !== undefined ? { BODY: tsvEscapeText(body).slice(0, 20000) } : {}),
        ...(p.mood !== undefined ? { MOOD: clamp1to10(p.mood) } : {}),
        ...(p.energy !== undefined ? { ENERGY: clamp1to10(p.energy) } : {}),
        ...(p.tags !== undefined ? { TAGS: String(p.tags || '').replace(/\t/g, ' ').slice(0, 120) || '-' } : {}),
        EDITED_AT: new Date().toISOString(),
      };
    }));  // no force needed: map() never changes row count, so the massacre guard's "half the rows gone" check can never trip here
    if (!found) throw new Error(`no journal entry ${id}`);
    auditLog.log('journal_entry_edited', { id });
  ...
  return { listEntries, addEntry, updateEntry, deleteEntry };
}
