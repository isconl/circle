# circle engine -- portable container build, same shape across every isconl
...
# 0.0.0.0 without a configured token. Set CIRCLE_TOKEN (or ISCONL_TOKEN) and
# CIRCLE_BIND=0.0.0.0 at runtime -- not baked into the image.
EXPOSE 8084
