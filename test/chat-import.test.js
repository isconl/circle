...
test('unzipTextFiles recurses into nested zip members (WhatsApp "export all chats" is a zip of zips)', () => {
  ...
  const outer = buildZip([
    { name: 'WhatsApp Chat with Taylor.zip', data: inner1 },
    { name: 'WhatsApp Chat with Alex.zip', data: inner2 },
  ]);
  const out = unzipTextFiles(outer);
  assert.equal(out.length, 2);
  assert.ok(out.some(f => f.name.includes('Taylor') && f.text === 'chat one'));
  assert.ok(out.some(f => f.name.includes('Alex') && f.text === 'chat two'));
});
...
test('importChat persists interleaved in/out messages to scope/inbox.tsv, PERSON_ID/DIRECTION tagged, for the matched person', async () => {
  ...
  const client = createChatImportClient({ ...store });
  const text = [
    '28/07/2026, 09:40 - Taylor Kariuki: hello there',
    '28/07/2026, 09:41 - You: hi Taylor',
  ].join('\n');
  const r = await client.importChat({ content: Buffer.from(text, 'utf8').toString('base64'), fileName: 'chat.txt' });
  assert.equal(r.newMessages, 2);
  const rows = store.data['scope/inbox.tsv'];
  assert.equal(rows.length, 2);
  assert.equal(rows.find(x => x.DIRECTION === 'in').PERSON_ID, 'taylor');
  assert.equal(rows.find(x => x.DIRECTION === 'in').SENDER, 'Taylor Kariuki');
  assert.equal(rows.find(x => x.DIRECTION === 'out').PERSON_ID, 'taylor');
...
test('importChat keeps each zip member as its own conversation, not joined, and re-running the same archive does not duplicate messages', async () => {
  const store = makeStore({ 'circle/people.tsv': [
    { ID: 'taylor', NAME: 'Taylor Kariuki', LAST_TOUCH: '-' },
    { ID: 'alex', NAME: 'Alex Rivera', LAST_TOUCH: '-' },
  ] });
  const client = createChatImportClient({ ...store });
  const zip = buildZip([
    { name: 'WhatsApp Chat with Taylor Kariuki.txt', data: Buffer.from('28/07/2026, 09:40 - Taylor Kariuki: hi', 'utf8') },
    { name: 'WhatsApp Chat with Alex Rivera.txt', data: Buffer.from('29/07/2026, 10:00 - Alex Rivera: hey', 'utf8') },
  ]);
