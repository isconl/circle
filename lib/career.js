 ...
 * is why asking it to explain "Re-ask Joel the unanswered question from 24 Jul" came
 * back as generic project-management advice: the model was handed a title and
 * nothing else. It did not know who Joel is, that he is the most important
 ...
 * `- name: Swenson Usjaerv ...` in the power map, `- id: D-001 ...` in the registers.
 */
...
function load(memoryDir = DEFAULT_MEMORY_DIR) {
  ...
  // Org files are overlaid on the org-neutral base. Peter's own principles are the
  ...
  const people = listBlocks(raw.powerMap, 'name').map(b => ({
    name:      b.value,
    // The short form everyone actually writes and says ("Swen" for Swenson
  ...
  const orgs = listBlocks(sectionByKey(raw.active, 'orgs'), 'id').map(b => ({
    ...
    role:   scalar(b.body, 'role'),
    status: scalar(b.body, 'status'),
    // Added for BC26082006 (org discovery). enabled defaults true -- an org
    // with no `enabled:` line at all (every org written before this field
    // existed) should read as enabled, not silently hidden.
    enabled: scalar(b.body, 'enabled') !== 'no',
    daysWorked: Number(scalar(b.body, 'days_worked') || 0),
    discoveryDate: scalar(b.body, 'discovery_date') || '',
    // The real OneDrive folder name discovery found this org under (e.g.
    // `2026-viva-valentia`, year prefix and all) -- distinct from `id`
    // (the slug alone) because BA26081803's OneDrive push needs to land in
    // the SAME folder discovery scanned, not a same-name-minus-year one.
    // Empty for a hand-added org with no discovery record.
    onedriveFolder: scalar(b.body, 'onedrive_folder') || '',
  })).filter(o => o.id);
...
function findCounterparties(title, people) {
  ...
  // Joel a pre-filled approver block so the CEO can sign it" is addressed to Joel;
  // the CEO is the reason, not the recipient.
  return hits.sort((a, b) => (a.matchedBy === 'named' ? 0 : 1) - (b.matchedBy === 'named' ? 0 : 1));
}

// Verbs that mean something has to leave Peter's desk and land in someone's inbox.
const COMMS_VERBS = [
 ...
 * "Reviewer-relief framing (Joel / Karmen)" - so the mapping lives in the vault
...
function contextBlock(resolved, { includeBodies = true } = {}) {
  ...
  if (resolved.org) L.push(`ORGANISATION: ${resolved.org}${resolved.role ? ` - Peter's role: ${resolved.role}` : ''}`);
...
function orient(tasks = [], today = new Date().toISOString().slice(0, 10)) {
  ...
  // Decisions still owed to Peter, and the ones he owes a written record for.
...
// ── WRITING (BC26082006 -- org discovery stub) ────────────────────────────────
// career.js was read-only until this row. The ONLY write this module does is
// appending a brand-new org's stub -- id/name/discovery_date, nothing else --
// never touching an org that already exists (existing entries are Sconl's by
// hand, this never overwrites them) and never touching any file other than
// _active.yaml + the new org's own org.yaml.

/**
 * Register a newly-discovered org as a stub, if it isn't already known.
 * Returns { created:true, id } or { created:false, reason } -- never throws
 * on "already exists", since the caller (vault's corporate-discovery pass)
 * re-runs this on every sync tick and a repeat isn't an error.
 */
function writeOrgStub(memoryDir, { id, name, discoveryDate, folder } = {}) {
  if (!id) throw new Error('writeOrgStub requires an id');
  const careerDir = path.join(memoryDir, 'career');
  const activeFile = path.join(careerDir, '_active.yaml');
  fs.mkdirSync(careerDir, { recursive: true });

  const existing = readIfPresent(activeFile);
  const existingIds = listBlocks(sectionByKey(existing, 'orgs'), 'id').map(b => b.value);
  if (existingIds.includes(id)) return { created: false, reason: 'already known', id };

  const today = discoveryDate || new Date().toISOString().slice(0, 10);
  const orgBlock = [
    `  - id: ${id}`,
    `    name: ${name || id}`,
    `    role: `,
  ...
  if (!existing) {
    next = `active_org: ${id}\norgs:\n${orgBlock}\n`;
  } else if (/^[ \t]*orgs:[ \t]*$/m.test(existing)) {
    next = existing.replace(/^([ \t]*orgs:[ \t]*)$/m, `$1\n${orgBlock}`);
  } else {
    next = `${existing.replace(/\s*$/, '')}\norgs:\n${orgBlock}\n`;
  }
  fs.writeFileSync(activeFile, next.replace(/\s*$/, '\n'));

  // Per-org stub: id/name/discovery_date only -- everything else (role,
  // power map, doctrine, decisions) is left for Sconl to fill in by hand,
  // per BC26082006's own scope. Never overwrites an existing org.yaml.
  const orgDir = path.join(careerDir, 'orgs', id);
  fs.mkdirSync(orgDir, { recursive: true });
  const orgYamlPath = path.join(orgDir, 'org.yaml');
  if (!fs.existsSync(orgYamlPath)) {
    fs.writeFileSync(orgYamlPath, `id: ${id}\nname: ${name || id}\ndiscovery_date: ${today}\nonedrive_folder: ${folder || id}\n`);
  }

  CACHE = null; // cheap insurance -- fingerprint mtime check would likely catch this anyway
