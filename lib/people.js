...
function createPeopleClient(opts) {
  ...
  async function writeDiaFile(id, sections) {
    ...
    // FI26091402: a missing dossier for THIS person is a legitimate first-time
    // creation (falls through to the skeleton below) -- but an EMPTY corpus
    // (zero .md files anywhere in diaDir) means the local checkout was never
    // mirrored from the live vault at all, and silently generating a fresh,
    // context-free dossier here would look plausible while being worthless.
    // Fail loudly instead so this is caught before it happens, not after.
    if (!readDiaFile(id)) {
      let corpusIsEmpty = false;
      try {
        corpusIsEmpty = !fs.readdirSync(diaDir).some((f) => f.endsWith('.md'));
      } catch (e) {
        corpusIsEmpty = true; // diaDir missing entirely -- same failure mode
      }
      if (corpusIsEmpty) {
        throw new Error(
          `writeDiaFile: refusing to generate a dossier for "${id}" -- diaDir (${diaDir}) has ` +
          `no existing .md files at all, which means this checkout's dossier corpus was never ` +
          `mirrored from the live vault. Proceeding would silently create a fresh, context-free ` +
          `dossier with no real touch history behind it. Mirror circle/memory/circle/dia/ from the ` +
          `live vault before running regenerate-dia here (see FI26091402/PI26091401).`
        );
      }
    }
    const people = await readTSV(peopleFile);
