...
const { norm: normName, findCandidates } = require('./fuzzy-match');

// A multi-line forwarded message (a WhatsApp list of links, one per line)
// carries raw \t/\n inside msg.text -- unlike inbox.js's addMessage, this
// file's row-building never ran that through an equivalent clean(), so a
// literal newline landed straight in a TSV cell and broke the row's column
// alignment on write (FI26082403, 24 Aug 2026: 9 rows in scope/inbox.tsv
// ended up with their real values shifted into the wrong columns and
// TITLE/SOURCE stuck at their '-' default). Same collapsing rule as
// inbox.js's clean() for consistency across every inbox writer.
 ...
 * flow (and Architect's own consolidated Downloads zip, 20 Aug) produces a
...
// BM26090503: how long an import's parsed-but-unmatched messages stay
// available in memory for a follow-up "Add to Circle" confirm -- long
// enough for Sconl to fill in the circle/role/cadence fields and click
// Add, short enough that a Node process restart or a genuinely stale tab
// doesn't leak memory forever. Re-running the import is the documented
// fallback once this expires (surfaced in the error message below), per
// this row's own "or worst case re-run" framing -- an OneDrive re-fetch of
// the archived file was scoped as a further fallback but not built this
// pass, since the in-memory path covers the real "just imported, now
// clicking Add" flow the row was written for.
const IMPORT_CACHE_TTL_MS = 20 * 60 * 1000;
...
function createChatImportClient(opts) {
  const {
    ...
    upsertPerson = null,                   // async (p) => {success, id} -- circle/lib/people.js, injected
    markAnalysisDirty = () => {},
    operatorNames = [],
    peopleFile = 'circle/people.tsv',
    interactionsFile = 'circle/interactions.tsv',
    inboxFile = 'scope/inbox.tsv',
    dismissedFile = 'circle/dismissed-matches.tsv',
  ...
  const importCache = new Map(); // importId -> { perFile, expiresAt }
  function cacheImport(perFile) {
    const id = `ci${Date.now()}${Math.random().toString(36).slice(2, 8)}`;
    for (const [k, v] of importCache) if (v.expiresAt < Date.now()) importCache.delete(k);
    importCache.set(id, { perFile, expiresAt: Date.now() + IMPORT_CACHE_TTL_MS });
    return id;
  }
  ...
  /** Everything that happens for ONE matched person -- interaction log +
   * LAST_TOUCH bump + per-message inbox rows -- factored out so the batch
   * loop in importChat() and the single-sender path in addUnmatchedSender()
   * (BM26090503) run identically instead of two hand-kept-in-sync copies. */
  async function settlePerson(m, { existingInbox, seenMsgKeys, nextIdNum }) {
    ...
    if (last?.date && /^\d{4}-\d{2}-\d{2}$/.test(last.date)) {
      ...
      if (!existing.some(x => (x.SUMMARY || '').includes(key))) {
        ...
        await appendTSV(interactionsFile, { ID: `X${Date.now()}${Math.random().toString(36).slice(2, 6)}`, PERSON_ID: m.person.ID,
    ...
    for (const msg of threadMsgs) {
      const cleanText = clean(msg.text);
      const source = `chatimport:${m.person.ID}:${msg.direction}:${msg.date}:${cleanText.slice(0, 40)}`;
      if (seenMsgKeys.has(source)) continue;
      seenMsgKeys.add(source);
      nextIdNum.n += 1;
      newRows.push({
        ID: `I${String(nextIdNum.n).padStart(3, '0')}`,
        TITLE: cleanText.slice(0, 70),
        BODY: cleanText,
        STATUS: 'seen',
        SOURCE: source,
    ...
    if (newRows.length) await rewriteTSV(inboxFile, rows => [...rows, ...newRows]);

    return r?.ok
      ? { id: m.person.ID, name: m.person.NAME, messages: m.msgs.length, updated: true }
      : { id: m.person.ID, name: m.person.NAME, messages: m.msgs.length, updated: false,
          note: r?.error || 'no private-capable model was available - the archive is filed, re-run the import later' };
  }

  /** BM26090503: create the Circle person Sconl just confirmed for an
   * unmatched sender, then run that ONE person through the exact same
   * population path a matched sender gets at import time -- using the
   * parsed archive still held in memory from that import (`importId`),
   * never re-parsing the whole .zip. Suggest-and-confirm: this only runs
   * when Sconl clicks "Add to Circle", never automatically. */
  async function addUnmatchedSender(p) {
    if (!upsertPerson) throw new Error('addUnmatchedSender requires an upsertPerson dependency');
    const entry = importCache.get(p.importId);
    if (!entry || entry.expiresAt < Date.now()) {
      throw new Error('This import has expired - re-run the chat import, then Add to Circle again.');
    }
    if (!String(p.speaker || '').trim()) throw new Error('which sender?');

    const norm = (s) => String(s || '').toLowerCase().replace(/[^a-z ]/g, '').trim();
    const sn = norm(p.speaker);
    let file = null;
    for (const f of entry.perFile) {
      if (Object.keys(f.parsed.bySpeaker).some(sp => norm(sp) === sn)) { file = f; break; }
    }
    if (!file) throw new Error(`"${p.speaker}" was not found in that archive anymore`);
    const msgs = Object.entries(file.parsed.bySpeaker).find(([sp]) => norm(sp) === sn)[1];

    const created = await upsertPerson({ name: p.name || p.speaker, circle: p.circle, role: p.role, cadence: p.cadence });
    const people = await readTSV(peopleFile);
    const person = people.find(row => row.ID === created.id);
    if (!person) throw new Error('person was created but could not be re-read');

    const existingInbox = await readTSV(inboxFile).catch(() => []);
    const seenMsgKeys = new Set(existingInbox.map(r => r.SOURCE));
    const nextIdNum = { n: existingInbox.reduce((n, r) => Math.max(n, parseInt(String(r.ID).replace(/\D/g, ''), 10) || 0), 0) };

    const result = await settlePerson({ person, speaker: p.speaker, msgs, file }, { existingInbox, seenMsgKeys, nextIdNum });
    markAnalysisDirty();
    auditLog.log('chat_import_unmatched_added', { personId: person.ID, messages: msgs.length });
    return { success: true, ...result };
  }

  /** BM26091205: Sconl confirmed a fuzzy suggestion -- the unmatched
   * speaker IS an existing contact, just under a different spelling.
   * Same settlePerson() pathway as a normal match / addUnmatchedSender,
   * just resolving to an EXISTING personId instead of creating a new one.
   * Never runs on its own -- only on an explicit confirm. */
  async function confirmMatchSuggestion(p) {
    const entry = importCache.get(p.importId);
    if (!entry || entry.expiresAt < Date.now()) {
      throw new Error('This import has expired - re-run the chat import, then try again.');
    }
    if (!String(p.speaker || '').trim()) throw new Error('which sender?');
    if (!String(p.personId || '').trim()) throw new Error('which existing contact?');

    const sn = normName(p.speaker);
    let file = null;
    for (const f of entry.perFile) {
      if (Object.keys(f.parsed.bySpeaker).some(sp => normName(sp) === sn)) { file = f; break; }
    }
    if (!file) throw new Error(`"${p.speaker}" was not found in that archive anymore`);
    const msgs = Object.entries(file.parsed.bySpeaker).find(([sp]) => normName(sp) === sn)[1];

    const people = await readTSV(peopleFile);
    const person = people.find(row => row.ID === p.personId);
    if (!person) throw new Error(`no contact with id ${p.personId}`);
    ...
    const result = await settlePerson({ person, speaker: p.speaker, msgs, file }, { existingInbox, seenMsgKeys, nextIdNum });
    markAnalysisDirty();
    auditLog.log('chat_import_suggestion_confirmed', { personId: person.ID, speaker: p.speaker, messages: msgs.length });
    return { success: true, ...result };
  }

  /** BM26091205: Sconl dismissed a fuzzy suggestion -- remembered so the
   * same (incoming name, existing contact) pair is never re-suggested.
   * Never removes or changes any contact data. */
  async function dismissMatchSuggestion(p) {
    if (!String(p.speaker || '').trim()) throw new Error('which sender?');
    if (!String(p.personId || '').trim()) throw new Error('which existing contact?');
    const key = normName(p.speaker);
    await appendTSV(dismissedFile, { SOURCE_KEY: key, PERSON_ID: p.personId, DISMISSED_AT: new Date().toISOString().slice(0, 10) });
    auditLog.log('chat_import_suggestion_dismissed', { sourceKey: key, personId: p.personId });
    return { success: true };
  }
  ...
  async function importChat(p) {
    ...
    // BM26091205: an unmatched speaker isn't necessarily a NEW person --
    // it might be an existing contact under a nickname/typo/transliteration.
    // Surface fuzzy candidates as an explicit suggestion, never an
    // automatic link. Excludes anyone already exactly matched this import
    // (matchSpeakers already claimed them) and any (name, personId) pair
    // Sconl has previously dismissed for this exact incoming name.
    const dismissedRows = await readTSV(dismissedFile).catch(() => []);
    const matchedIds = new Set(allMatched.map(m => m.person.ID));
    for (const u of allUnmatched) {
      const key = normName(u.speaker);
      const dismissed = new Set(dismissedRows.filter(r => r.SOURCE_KEY === key).map(r => `${key}::${r.PERSON_ID}`));
      u.suggestions = findCandidates(u.speaker, people, { excludeIds: matchedIds, dismissed });
    }
    ...
    for (const m of allMatched.slice(0, 100)) {
      const before = seenMsgKeys.size;
      results.push(await settlePerson(m, { existingInbox, seenMsgKeys, nextIdNum }));
      newMessageCount += seenMsgKeys.size - before;
    }

    // BM26090503: every unmatched sender's messages stay in memory (keyed by
    // this importId) so "Add to Circle" can settle them without re-parsing
    // the archive -- see addUnmatchedSender() above.
    const importId = cacheImport(perFile);

    markAnalysisDirty();
    auditLog.log('chat_archive_imported', { messages: totalMessages, people: results.length, filed: !!filed, newMessages: newMessageCount });
    return { success: true, filed, files: perFile.length, messages: totalMessages,
      newMessages: newMessageCount, fileNames: texts.map(t => t.name), importId,
      updated: results, unmatched: allUnmatched.sort((a, b) => b.count - a.count) };
  }

  return { importChat, addUnmatchedSender, confirmMatchSuggestion, dismissMatchSuggestion };
}
