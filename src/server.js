...
async function main() {
  ...
  // Speaker names that mean "Sconl himself", so his own messages get
  // excluded from matching instead of showing up as an unmatched speaker
  // (found live 18 Aug: "Sconl" had 26 unmatched messages in a real import
  // because this was never configured -- parseChatExport() always accepted
  // an operatorNames list, nothing ever passed one in).
  const OPERATOR_NAMES = (process.env.CIRCLE_OPERATOR_NAMES || 'Sconl').split(',').map(s => s.trim()).filter(Boolean);
