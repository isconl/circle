...
function createPeopleClient(opts) {
  ...
  function listPeople() {
    const people = readTSV(peopleFile);
    const touches = readTSV(interactionsFile);
  ...
  async function upsertPerson(p) {
    ...
    if (p.id && rows.some(r => r.ID === id)) {
      rewriteTSV(peopleFile, all => all.map(r => r.ID === id ? { ...r,
        ...
        CADENCE_DAYS: clean(p.cadence || r.CADENCE_DAYS), NOTE: clean(p.note || r.NOTE) } : r));
    ...
    appendTSV(peopleFile, { ID: id, NAME: clean(p.name), CIRCLE: circle,
      ...
      NOTE: clean(p.note), REMEMBER: clean(p.remember) });
  ...
  function setRemember(p) {
    if (!p.id) throw new Error('which person?');
    let found = false;
    rewriteTSV(peopleFile, rows => rows.map(r => {
  ...
  function logTouch(p) {
    ...
    appendTSV(interactionsFile, { ID: `X${Date.now()}`, PERSON_ID: p.personId, DATE: date,
      CHANNEL: clean(p.channel), SUMMARY: clean(p.summary), NEXT: clean(p.next), CREATED_AT: new Date().toISOString().slice(0, 10) });
    rewriteTSV(peopleFile, rows => rows.map(r => r.ID === p.personId ? { ...r, LAST_TOUCH: date } : r));
  ...
  function whoCan(q) {
    ...
    const people = readTSV(peopleFile);
    const caps = readTSV(capabilitiesFile);
    const edges = readTSV(graphFile);
